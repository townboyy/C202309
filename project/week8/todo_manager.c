#include <stdio.h>
#define MAX_TASKS 10 //변수 정의
#define CHAR_NUM 100
#include <string.h>

int main() { //메인 함수
	char tasks[MAX_TASKS][CHAR_NUM] = { "" }; // 할 일 목록을 저장하기 위한 2차원 배열 // 변수 선언
	...
	while (1) {
		...
		// 입력에 따른 기능 수행
		switch (choice) { //switch 조건문
		case 1: //1을 입력받으면 할 일을 입력받고 저장
		...
		case 2:
			// 할 일 삭제하는 코드 블록 
			printf("삭제할 할 일의 번호를 입력해주세요. (1부터 시작):"); //삭제 할 일의 번호 입력받고 삭제, delindex에 저장
			scanf_s("%d", &delIndex);
			if (delIndex > taskCount || delIndex <= 0) { //delindex가 taskcount보다 크면 삭제범위 벗어나서 삭제 못함 출력.
				printf("삭제 범위가 벗어났습니다.\n");
			}
			else {//그렇지 않으면 입력한 일 삭제
